...
func (s *authService) Logout(c *fiber.Ctx, req *validation.Logout) error {
	...
	err = s.TokenService.DeleteToken(c, config.TokenTypeRefresh, token.UserID)
...
func (s *authService) ResetPassword(c *fiber.Ctx, query *validation.Token, req *validation.UpdatePassOrVerify) error {
	...
	if errUpdate := s.UserService.UpdatePassOrVerify(c, req, user.ID); errUpdate != nil {
		return errUpdate
	}

	if errToken := s.TokenService.DeleteToken(c, config.TokenTypeResetPassword, user.ID); errToken != nil {
		return errToken
	}
...
func (s *authService) VerifyEmail(c *fiber.Ctx, query *validation.Token) error {
	...
	if errUpdate := s.UserService.UpdatePassOrVerify(c, &validation.UpdatePassOrVerify{}, user.ID); errUpdate != nil {
		return errUpdate
	}

	if errToken := s.TokenService.DeleteToken(c, config.TokenTypeVerifyEmail, user.ID); errToken != nil {
		return errToken
	}
