      ...
      <main className="flex-1 container mx-auto px-4 md:px-0 pt-4 pb-6 max-w-4xl">
        ...
        <Card className="mb-4 p-0 md:p-8 flex flex-col md:flex-row gap-2 md:gap-8 shadow-lg border-0 bg-white rounded-2xl">
          ...
          <div className="flex-1 flex flex-col gap-1 md:gap-2 justify-center px-4 md:px-0 md:mx-0">
            <div className="flex items-center gap-2 mb-1 md:mb-4">
              <h1 className="text-xl md:text-3xl font-bold break-words leading-tight flex-1">
                {product.name}
              </h1>
              ...
              <span className="flex items-center gap-1">
                <span className="text-orange-500">
                  <svg
                    ...
                    viewBox="0 0 20 20"
                  >
                    <path d="M10 2a6 6 0 016 6c0 4.418-6 10-6 10S4 12.418 4 8a6 6 0 016-6zm0 8.5A2.5 2.5 0 1010 5a2.5 2.5 0 000 5.5z" />
                  </svg>
                </span>
                {product.sold ?? 0} Sold
              </span>
            ...
            <div className="flex items-end gap-2 bg-gray-50 rounded-lg px-3 py-2 mb-2">
              ...
              <span
                className={`ml-2 px-2 py-1 rounded text-xs font-semibold ${product.quantity === 0 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-700'}`}
              >
                {product.quantity === 0
                  ? 'Out of stock'
            ...
            <div className="block md:hidden fixed bottom-0 left-0 w-full z-40 px-4 pb-4 pointer-events-none">
              <div className="pointer-events-auto">
                <AddToCartButton
                  product={product}
                  className="w-full rounded-xl shadow-lg py-4 text-lg font-semibold"
        ...
        <div className="mt-6 mb-4 md:mt-10 md:mb-8 bg-white rounded-xl shadow p-4 md:p-6 max-w-3xl mx-auto">
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="flex items-center gap-0.5">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-4 h-4 ${i < review.rating ? 'fill-yellow-400 stroke-yellow-500' : 'stroke-gray-300'}`}
                        />
                      ))}
                    </span>
                  </div>
          ...
          >
            <div className="mb-3">
              ...
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <Star
                      className={`w-7 h-7 ${reviewRating >= star ? 'fill-yellow-400 stroke-yellow-500' : 'stroke-gray-300'}`}
                    />
                  </button>
                ))}
              </div>
            ...
            {submitMsg && (
              <div className="mb-2 text-sm text-green-600">{submitMsg}</div>
            )}
            <button
              type="submit"
      ...
      <div className="overflow-x-auto max-w-full scroll-smooth snap-x px-2 pb-1 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
        <div className="flex gap-2 w-max">
          {moreImages.map((img: string, i: number) => (
            <button
              key={img}
              className="relative w-28 h-20 flex-shrink-0 rounded-lg overflow-hidden border border-gray-200 group snap-start"
              onClick={() => {
                setSelectedIdx(i);
                setOpen(true);
              }}
      ...
      <div className="bg-white rounded-b-xl shadow-sm px-3 py-4 min-h-[60px] text-sm md:text-base">
        ...
        {tab === 'ingredients' && (
          <div className="text-muted-foreground">
            100% Pure Bird&apos;s Nest, no additives.
          </div>
        )}
        {tab === 'usage' && (
          <div className="text-muted-foreground">
            Soak in water for 30 minutes, cook with sugar or as desired.
          </div>
        )}
        {tab === 'preservation' && (
          <div className="text-muted-foreground">
